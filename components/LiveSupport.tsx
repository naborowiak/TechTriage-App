...
const Button: React.FC<{ 
  ...
  const themes = {
    ...
    gold: "bg-cta-500 hover:bg-cta-600 text-white shadow-lg shadow-cta-500/20", // Mapped gold to CTA (Orange) for brand consistency
...
const endSessionTool: FunctionDeclaration = {
  ...
  parameters: {
    type: Type.OBJECT,
    description: 'Call this when the support session is finished or resolved.',
    properties: {
      summary: { type: Type.STRING, description: 'A 1-sentence summary of the fix.' }
    },
    required: ['summary']
  }
};
...
export const LiveSupport: React.FC<LiveSupportProps> = ({ onClose }) => {
  ...
  const drawWaveform = () => {
    ...
    for (let i = 0; i < bufferLength; i++) {
      ...
      ctx.fillStyle = currentlySpeaking ? '#F97316' : '#1F2937'; // CTA Orange or Brand Navy
  ...
  useEffect(() => {
      ...
      try {
        ...
        const sessionPromise = ai.live.connect({
          ...
          config: {
            ...
            systemInstruction: "You are TechTriage Live. You are a professional tech co-pilot. Assist the user in real-time."
          },
          callbacks: {
            onopen: () => {
                if (mounted) { 
                ...
                processor.onaudioprocess = (e) => {
                    if (isMutedRef.current || isSessionEnded) return;
                    const pcm = new Int16Array(e.inputBuffer.length);
                    const chan = e.inputBuffer.getChannelData(0);
                    for(let i=0; i<chan.length; i++) pcm[i] = chan[i] * 32768;
                ...
                frameIntervalId = window.setInterval(() => {
                    if (videoRef.current && canvasRef.current && mounted && !isSessionEnded) {
                        ...
                        if (ctx && videoRef.current.videoWidth > 0) {
                            ...
                            canvas.toBlob(blob => {
                                if (blob) {
                                    ...
                                    reader.onloadend = () => sessionPromise.then(s => s.sendRealtimeInput({ 
                                      media: { 
            ...
            onmessage: async (msg: LiveServerMessage) => {
                ...
                if (msg.toolCall) {
                    const call = msg.toolCall.functionCalls.find(f => f.name === 'endSession');
                    if (call) { 
                      const finalSummary = call.args.summary as string;
                      setSummary(finalSummary); 
                      setIsSessionEnded(true); 
                      stopAllHardware(); 
                      archiveSession(finalSummary, transcriptRef.current);
                    }
                }
            ...
            onclose: () => { if (mounted && !isSessionEnded) onClose(); }
          }
        });
    ...
    start();
    return () => { mounted = false; clearInterval(frameIntervalId); stopAllHardware(); };
  }, []); 
