...
export const SessionHistory: React.FC<SessionHistoryProps> = ({ onBack, userEmail: _userEmail, userName, embedded = false, onReopenCase }) => {
  ...
  const [caseMessages, setCaseMessages] = useState<Array<{ role: string; text: string; image?: string; timestamp: number; agentName?: string }>>([]);
  ...
  const downloadCaseGuide = (caseData: CaseRecord) => {
    ...
    doc.text('TotalAssist', margin, 25);
    doc.setFontSize(10);
    ...
    doc.setTextColor(107, 114, 128);
    doc.setFontSize(8);
    doc.text('This report was generated by TotalAssist.', margin, yPosition);

    doc.save(`TotalAssist_Case_${caseData.id.substring(0, 8)}_${new Date(caseData.createdAt).toISOString().split('T')[0]}.pdf`);
  };
          ...
          {isLoading ? (
            ...
            <div className="bg-white dark:bg-midnight-800 rounded-2xl p-8 text-center border border-gray-100 dark:border-midnight-700">
              ...
              <p className="text-text-muted text-sm">
                Your support cases will appear here after you start a support session.
              </p>
            </div>
          ) : (
            filteredCases.map(c => (
