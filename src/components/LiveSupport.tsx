...
export const LiveSupport: React.FC<LiveSupportProps> = ({
  ...
  const generateSessionGuidePDF = (): string => {
    ...
    for (const entry of transcriptHistory) {
      ...
      const speaker = entry.role === "user" ? "You" : "Support";
      doc.setFont("helvetica", "bold");
      doc.setFontSize(9);
    ...
    doc.setFontSize(8);
    doc.text(
      "This guide was automatically generated by TotalAssist.",
      margin,
      yPosition,
    );
    yPosition += 4;
  ...
  const handleDownloadGuide = () => {
    ...
    a.href = url;
    a.download = `TotalAssist_Guide_${new Date().toISOString().split("T")[0]}.pdf`;
  ...
  const handleDownloadReport = () => {
    ...
    const body = transcriptHistory
      ...
      .join("\n\n");
    const footer = `\n\n--- END OF REPORT ---\nSecurity Verified By TotalAssist`;

    const blob = new Blob([header + body + footer], { type: "text/plain" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = `TotalAssist_Report_${Date.now()}.txt`;
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
    URL.revokeObjectURL(url);
              ...
              >
                <div className="flex items-center gap-2 mb-2 opacity-40">
                  ...
                  <span className="text-[9px] font-bold uppercase tracking-widest text-white">
                    {entry.role === "model" ? "Support" : (userName || "You")}
                  </span>
                </div>
                <div
