...
import { GoogleGenAI, Modality, LiveServerMessage, Type, Session } from '@google/genai';
...
const BotAvatar = ({ className }: { className: string }) => {
  ...
  return <img src="/tech-triage-logo.png" className={`${className} object-contain`} alt="AI" onError={() => setError(true)} />;
};
...
export const LiveSupport: React.FC<LiveSupportProps> = ({ onClose }) => {
  ...
  const addTranscriptEntry = useCallback((role: 'user' | 'model', text: string) => {
    if (!text.trim()) return;
    setTranscriptHistory(prev => [...prev, { role, text, timestamp: Date.now() }]);
  }, []);
  ...
  const archiveSession = useCallback((finalSummary: string, history: TranscriptEntry[]) => {
  ...
  const drawWaveform = useCallback(() => {
    ...
    const currentlySpeaking = playbackCtx && playbackCtx.currentTime < (nextPlayTimeRef.current + 0.15);
  ...
  const endSession = useCallback((finalSummary?: string) => {
    stopAllHardware();
...
You have access to a tool called endSession that you should call when the user indicates they're done or the issue is resolved. Include a brief summary of what was accomplished.`,
        ...
        if (mounted) {
          setConnectionError(e instanceof Error ? e.message : 'Failed to start session');
          setIsConnecting(false);
        }
      }
    };
  ...
  }, [drawWaveform, playAudioData, addTranscriptEntry, endSession, stopAllHardware]);
         ...
         {isConnecting && !connectionError && (
           <div className="absolute inset-0 flex items-center justify-center z-50 bg-brand-900/60">
             <div className="text-center">
               ...
               <p className="text-white font-bold uppercase tracking-widest text-xs">Connecting to AI Agent...</p>
             </div>
           </div>
         )}
