...
export const SettingsProfileSection: React.FC<ProfileSectionProps> = ({ user, onUpdateUser }) => {
  ...
  const handleSave = () => {
    ...
    const existing = JSON.parse(localStorage.getItem('totalassist_settings') || '{}');
    localStorage.setItem('totalassist_settings', JSON.stringify({ ...existing, phone }));
...
export const SettingsNotificationsSection: React.FC = () => {
  ...
  const handleToggle = (key: string, value: boolean) => {
    const existing = JSON.parse(localStorage.getItem('totalassist_settings') || '{}');
    localStorage.setItem('totalassist_settings', JSON.stringify({ ...existing, [key]: value }));
  };
        ...
        <label className="flex items-center justify-between cursor-pointer">
          ...
          <div className="relative">
            <input
              ...
              onChange={(e) => { setEmailNotifications(e.target.checked); handleToggle('emailNotifications', e.target.checked); }}
        ...
        <label className="flex items-center justify-between cursor-pointer">
          ...
          <div className="relative">
            <input
              ...
              onChange={(e) => { setSessionGuideEmails(e.target.checked); handleToggle('sessionGuideEmails', e.target.checked); }}
              className="sr-only peer"
            />
            <div className="w-11 h-6 bg-gray-200 dark:bg-midnight-600 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-electric-indigo"></div>
          </div>
...
// ==========================================
// Settings Component (composes sections)
// ==========================================
